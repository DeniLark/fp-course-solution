...
instance Functor k => Functor (StateT s k) where
  ...
  f <$> StateT sta = StateT $ ((\(a, s) -> (f a, s)) <$>) . sta
...
instance Monad k => Applicative (StateT s k) where
  ...
  StateT stf <*> StateT sta =
    StateT $ \s1 -> stf s1 >>= \(f, s2) -> sta s2 >>= \(a, s3) -> pure (f a, s3)
...
instance Monad k => Monad (StateT s k) where
  ...
  k =<< StateT sta = StateT $ \s1 -> sta s1 >>= \(a, s2) -> runStateT (k a) s2
...
distinct' as = eval'
  (filtering
    (\a -> getT >>= \set -> putT (S.insert a set) >> pure (S.notMember a set))
    as
  )
  S.empty
...
distinctF as = evalT
  (filtering
    (\a -> if a > 100
      ...
      else getT >>= \set -> putT (S.insert a set) >> pure (S.notMember a set)
    )
    as
  )
  S.empty
...
instance Monad k => Monad (OptionalT k) where
  ...
  k =<< OptionalT opta = OptionalT $ opta >>= onFull (runOptionalT . k)
...
instance Monad (Logger l) where
  ...
  k =<< Logger ls1 a = let Logger ls2 b = k a in Logger (ls1 ++ ls2) b
...
onFull
  :: Applicative k => (t -> k (Optional a)) -> Optional t -> k (Optional a)
