...
instance Functor (State s) where
  ...
  f <$> State st = State $ \s1 -> let (a, s2) = st s1 in (f a, s2)
...
instance Monad (State s) where
  ...
  k =<< State st = State $ \s1 -> let (a, s2) = st s1 in runState (k a) s2
...
findM p = foldRight (\a acc -> p a >>= bool acc (pure $ Full a)) (pure Empty)
...
firstRepeat as = eval
  (findM (\a -> get >>= \set -> put (S.insert a set) >> pure (S.member a set))
         as
  )
  S.empty
...
distinct as = eval
  (filtering
    (\a -> get >>= \set -> put (S.insert a set) >> pure (S.notMember a set))
    as
  )
  S.empty
...
square =
  foldRight (\a -> let x = digitToInt a in (toInteger (x * x) +)) 0 . show'
