...
jsonString = between (is '"') (charTok '"') (list pc)
 where
  pc :: Parser Char
  pc = character >>= pcc

  pcc :: Char -> Parser Char
  pcc '\\' = character >>= \ch -> if ch == 'u'
    then hex
    else optional (pure . fromSpecialCharacter)
                  (unexpectedCharParser ch)
                  (toSpecialCharacter ch)
  pcc '"' = unexpectedCharParser '"'
  pcc ch  = pure ch
...
jsonNumber = P $ \i ->
  optional (Result <$> snd <*> fst) (UnexpectedString i) (readFloats i)
