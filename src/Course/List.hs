...
seqOptional = foldRight foldFunc (Full Nil)
 where
  foldFunc :: Optional a -> Optional (List a) -> Optional (List a)
  foldFunc (Full a) (Full as) = Full $ a :. as
  foldFunc _        _         = Empty
...
find p = foldRight (\a -> if p a then const $ Full a else id) Empty
