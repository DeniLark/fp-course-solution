...
(|||) pa pb = P $ \input ->
  let res = parse pa input in if isErrorResult res then parse pb input else res
...
instance Monad Parser where
  ...
  k =<< pa = P $ \input -> case parse pa input of
    Result input' a    -> parse (k a) input'
...
satisfy p = (bool unexpectedCharParser pure =<< p) =<< character
  -- (\ch -> bool (unexpectedCharParser ch) (pure ch) $ p ch) =<< character
...
sequenceParser = foldRight (lift2 (:.)) (pure Nil)
  -- foldRight (\pa pacc -> pa >>= \a -> pacc >>= pure . (a :.)) (pure Nil)
...
smokerParser = False <$ is 'n' ||| True <$ is 'y'
...
phoneBodyParser =
  (++) <$> list digit <*> ((:.) <$> (is '-' ||| is '.') <*> list digit) ||| pure
    ""
...
phoneParser = (++) <$> phoneBodyParser <*> phoneBodyParser <* is '#'
